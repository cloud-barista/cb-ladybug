...
get() {
	...
	$APP_ROOT/src/grpc-api/cbadm/cbadm cluster get --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --cluster ${v_CLUSTER} | jq -r ".clusterConfig" > kubeconfig.yaml
	echo "kubectl get nodes --kubeconfig=./kubeconfig.yaml --insecure-skip-tls-verify=true"
#	chmod 400 ${v_CLUSTER}.pem
#	cat ${v_CLUSTER}.pem
}
