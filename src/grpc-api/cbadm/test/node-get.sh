...
# usage
if [ "$#" -lt 1 ]; then 
	echo "node-get.sh [GCP/AWS] <clsuter name> <node name>"
	echo "    ./node-get.sh GCP cb-cluster cb-gcp-cluster-test-0-w-asdf"
	exit 0; 
fi
...
get() {
	...
	$APP_ROOT/src/grpc-api/cbadm/cbadm node get --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --cluster ${v_CLUSTER_NAME} --node ${v_NODE_NAME}
