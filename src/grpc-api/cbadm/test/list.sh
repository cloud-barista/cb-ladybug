...
# usage
if [ "$1" == "-h" ]; then 
	echo "./list.sh [GCP/AWS] [all/config/ns/vpc/fw/mcis/ssh]"
	echo "./list.sh GCP ns"
	echo "./list.sh GCP config,ns"
	exit 0
fi
...
if [ "${v_QUERY}" == "" ]; then 
	read -e -p "Query ? [all/config/ns/vpc/fw/mcis/ssh] : "  v_QUERY
fi
if [ "${v_QUERY}" == "" ]; then echo "[ERROR] missing <query>"; exit -1; fi
if [ "${v_QUERY}" == "all" ]; then v_QUERY="config,ns,vpc,fw,mcis,ssh"; fi
