...
# usage
if [ "$#" -lt 1 ]; then 
	echo "node-list.sh [GCP/AWS] <clsuter name>"
	echo "    ./node-list.sh GCP cb-cluster"
	exit 0; 
fi
...
list() {
	...
	$APP_ROOT/src/grpc-api/cbadm/cbadm node list --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --cluster ${v_CLUSTER_NAME}
