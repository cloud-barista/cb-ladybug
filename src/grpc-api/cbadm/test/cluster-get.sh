...
# usage
if [ "$#" -lt 1 ]; then 
	echo "cluster-get.sh [GCP/AWS] <clsuter name>"
	echo "    ./cluster-get.sh GCP cb-cluster"
	exit 0; 
fi
...
get() {
	...
	$APP_ROOT/src/grpc-api/cbadm/cbadm cluster get --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --cluster ${v_CLUSTER_NAME}
