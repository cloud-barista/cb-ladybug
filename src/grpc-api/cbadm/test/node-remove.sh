...
delete() {
	...
	$APP_ROOT/src/grpc-api/cbadm/cbadm node remove --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --cluster ${v_CLUSTER_NAME} --node ${v_NODE_NAME}
